'''
NOTE: Comment out the right instructions and tools for the guest agent in the guest_agent.py file for the right ablation experiment.
We are running exp4 which is the guest simulation agentic system with just the message attributes generation agent. 
This system only has access to the guest name, target order, past messages in the conversation so far and message attributes generation through its tools.
This system does not have order tracking.
'''
...
class ConversationClient:
    ...
    async def run(self):
        ...
        guest_agent_message = start_message
        ...
        while turn_count < MAX_TURNS:
            try:
                ...
                order_taking_llm_response = await get_order_taking_llm_response(
                    order_taking_llm_handler, guest_agent_message
                )
                ...
                guest_agent_deps.past_messages_history = self.get_chat_history_as_string()
                guest_agent_response = await run_agent(guest_agent, order_taker_response_content, guest_agent_deps)
                print(
                ...
                guest_agent_response_output = guest_agent_response.output
                ...
                guest_agent_message = Message(role="agent", name=VALIDATION_AGENT_ROLE_NAME, content=guest_agent_response_output, tool_calls=tool_call_latencies, usage=usage_dict)
                self.chat_history.append(guest_agent_message)
                ...
                # recent_guest_messages.append(guest_agent_response_output.lower())
                ...
                if guest_agent_response_output.lower() in ["quit", "exit", "q"]:
                    break
                ...
                print(error_message)
                break
        self.chat_history.append(Message(role="system", name='test_case_info', content=json.dumps(self.test_case)))
        self.get_chat_messages_log().to_csv(
            f"logs/chat_with_{VALIDATION_AGENT_ROLE_NAME}_messages_log_{self.test_case_name}_test_case_number_{self.test_case_index+1}.csv",
            index=False,
        )
